...
{
    public partial class Annuities : Form
    {
        public Annuities()
        {
            InitializeComponent();
        ...
        {
            OutputlistBox.Items.Clear();
            double interestRate, periodicRate, period, payment;
                    ...
                    if (double.TryParse(RatePerPeriodtextBox.Text, out periodicRate))
                    {
                        if (double.TryParse(PeriodtextBox.Text, out period))
                        {
                            if (double.TryParse(InputIntRatetextBox.Text, out interestRate))
                            {
                                ...
                                interestRate = interestRate / 100;
                                double AV = ((payment * (1 + interestRate)) * (((Math.Pow((1 + interestRate), (period * periodicRate))) - 1) / interestRate));
                ...
                if (double.TryParse(RatePerPeriodtextBox.Text, out periodicRate))
                {
                    if (double.TryParse(PeriodtextBox.Text, out period))
                    {
                        if (double.TryParse(InputIntRatetextBox.Text, out interestRate))
                        {
                            ...
                            {
                                interestRate = interestRate / 100;
                                double AV = (payment * (((Math.Pow((1 + interestRate), (period * periodicRate))) - 1) / interestRate));
                                Outputlabel.Text = "Accumulated value:";
                                OutputTextBox.Text = AV.ToString("n5");
                                int n = (int)period;
                                double residual = period - n;
