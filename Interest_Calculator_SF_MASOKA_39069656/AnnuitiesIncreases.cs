...
{
    public partial class AnnuitiesIncreases : Form
    {
        public AnnuitiesIncreases()
        {
            InitializeComponent();
        }
        ...
        {
            OutputlistBox.Items.Clear();
            double interestRate, periodicRate, period, principle, payment,increasingRate;
                        ...
                        if (double.TryParse(InputPrincipletextBox.Text, out principle))
                        {
                            if (double.TryParse(RatePerPeriodtextBox.Text, out periodicRate))
                            {
                                if (double.TryParse(PeriodtextBox.Text, out period))
                                {
                                    if (double.TryParse(InputIntRatetextBox.Text, out interestRate))
                                    {
                                        if (double.TryParse(IncreasingRatetextBox.Text, out increasingRate))
                                        {

                                            interestRate = interestRate / 100;
                                            increasingRate = increasingRate / 100;
                                            double AV = (principle * (Math.Pow((1 + interestRate), (periodicRate * period)))) + ((payment * (1 + interestRate)) * ( ( ( Math.Pow((1+interestRate),(period*periodicRate)))-(Math.Pow((1 + increasingRate), (period * periodicRate))))/(interestRate-increasingRate)));
                                            ...
                                            for (int x = 0; x < n; x++)
                                            {
                                                ...
                                                { 
                                                    sum = (sum + pp) + ((sum+pp)*interestRate);
                                                    pp = pp + (pp * increasingRate);
                                                }
                                                int f = x + 1;
                                                for (int r = 0; r < periodicRate; r++)
                                ...
                                if (double.TryParse(InputIntRatetextBox.Text, out interestRate))
                                {
                                    if (double.TryParse(PaymentTextBox.Text, out payment))
                                    {
                                        if (double.TryParse(IncreasingRatetextBox.Text, out increasingRate))
                                        {
                                            interestRate = interestRate / 100;
                                            increasingRate = increasingRate / 100;
                                            double AV = (principle * (Math.Pow((1 + interestRate), (periodicRate * period)))) + (payment * (((Math.Pow((1 + interestRate), (period * periodicRate))) - (Math.Pow((1 + increasingRate), (period * periodicRate)))) / (interestRate - increasingRate)));
                                            Outputlabel.Text = "Accumulated value:";
                                            OutputTextBox.Text = AV.ToString("n5");
                                            int n = (int)period;
                                            double residual = period - n;
                                            ...
                                            for (int x = 0; x < n; x++)
                                            {
                                                ...
                                                for (int r = 0; r < periodicRate; r++)
                                                {
                                                    ...
                                                    if (abc)
                                                    {
                                                        ...
                                                        payment = payment + (payment * increasingRate);
                                                    }
                                                    abc = true;
                                                    //payment = payment + (payment * increasingRate);
                                                }
